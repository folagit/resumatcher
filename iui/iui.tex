...
This section we survey the previous research on the job recommender systems. Recommender Systems are software tools and techniques providing suggestions for items to be of use to a user~\cite{ricci2011introduction}. There have been several attempts to use techniques of recommender system to improve the process of job finding.

Rafter et al. \cite{rafter2000personalised} began to use ACF (Automated Collaborative Filtering) in their Job Recommender system, ``CASPER''. In the system user profiles are gotten from server logs, from system logs, the system will recommend jobs to users that other similar users had reviewed. But since the searching result is huge, and the result is sorted randomly, even two very similar users may review different jobs. Some hybrid recommender systems had been presented, like F{\"a}rber et al \cite{farber2003automated},  presented a recommender system built on a hybrid approach. The system integrated two methods, content-based filtering and collaborative filtering. They tried to overcome the problem of rating data sparsity by leveraging synergies of a combined model, et. the  latent aspect model. Daramola et al. \cite{daramola2010fuzzy} proposed a fuzzy logic based expert system(FES) tool for online personnel recruitment. The system use a fuzzy distance metric to rank candidates' profile in the order of their eligibility for the job. However, most of these studies used synthetic data, and not on real resumes and job descriptions. 
...
How calculate similarity between a resume and a job description is another important problem of content-based job recommender systems. 

S{\'a}nchez et al. \cite{sanchez2012ontology} summarized ontology-based similarity assessment into three kinds, and advantages and disadvantages of each approach. The three kinds of categories are: Edge-counting approaches, Feature-based measures, and Measures based on Information Content. The edge-counting approaches view the ontology as a directed graph in which concepts are nodes, so the method to calculate similarity is to compute the minimum path length between nodes. But when they meet multiple taxonomical inheritance, the several taxonomical paths are not taken into account. Feature based approaches assess similarity between concepts as a function of their properties. However properties of ontology are not always available, so the applicability of these approaches are limited. Measures based on Information Content depends on the amount of shared information between two terms, a dimension which is represented by their Least Common Subsumer (LCS) in an ontology. The approaches require big, and fine grained ontologies with a detailed taxonomical structure.
...
Sometimes the job finding web sites want users to input their personal information, like education, working experience details, and skills. However, in real life, job seekers do not want to input a lot information manually~\cite{singh2010prospect}. To create the models from Job Descriptions and Resumes, the system need to extract information from resumes and job descriptions. Both resumes and job descriptions are written in natural language, so the challenge is how to extract information from these unstructured or semi-structured data source, and transfer them into structured data.
...
\begin{tabular}{   | p{8cm} |  }
 ...
 \hline
\end{tabular}
\label{tab:onto_patterns} % is used to refer this table in the text
\end{table}
...
In this section, we will test the performance of key components of system, and then the whole system. The Information Extraction and the Ontology Similarity modules are two important part of the system.  We will evaluate them respectively.

\subsection{Experiments of Information Extraction}

To evaluate the performance of information extraction, we get some sentences by the sentence filters as the test data. Different sentence filter can extract different kinds of sentences. We use degree sentences to explain the process:
...
Table ~\ref{tab:dismatrix1} shows similarity values between terms, which is calculated from  from 500 job descriptions. The values in the table are the similarities between terms, the higher the value, the greater of the similarity, so the similarity of one term itself is one. We select one concept, rank other concepts by their similarity value to this concept. The ranked concepts are also be given relevance scores by human judge, so we can use NDCG to evaluate the effectiveness of our approach.
...
We first used Precision@K to compare the performance our approach to classical information retrieval models. They are Okapi BM25~\cite{robertson2009probabilistic}, Kullback-Leibler divergence, and TF-IDF. Precision@K is the proportion of relevant documents in the first K positions and is given below:
