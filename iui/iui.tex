...
\end{table} 
 ...
 Based on such observation, we give a new statistical-based Ontology Similarity Measure. Two concepts can get similarity If the two concepts $a$ and $b$ have the same direct hypernym or one  is the hypernym of the other one, their similarity $S_{a,b}$ could be the ratio of two factors:
...
Before calculate the similarity between concepts, we need construct the Ontology at first.  Semantic web have been a hot research topic in these years, thousands of domain ontologies had been -created~\cite{ding2004swoogle}. A paradigmatic example is WordNet~\cite{fellbaum1998wordnet}, a general purpose thesaurus, which contains more than 100,00 general English concepts.  Currently, there is no technology ontology build for recruiting purpose.  ACM has created a poly-hierarchical ontology that can be utilized in semantic web applications~\cite{acm2012class}, but it is mostly used in academic area.
...
For example, we extract the word  ''XSL'', which currently not in the skill set. We check the word on DBpedia, get the XML formatted description from URL : http://dbpedia.org/page/XSL. If any of ``dcterms:subject'' fields have the value which is the technical category,  like ``Programming languages'', ``Markup languages'', we  indicate  the word is a technical term, and pop them up. Such iteration will continue until the number of new terms getting below a threshold. The process is shown Figure ~\ref{fig:gen_onto}
