...
The content-based recommender systems need the structured information of items, but the items in JRS are resumes and job descriptions are written in natural language, so how to get information from the documents is a big challenge. In some JRSs the users are asked to input personal information, like education and experience details, skills to the system. However, in real life, the job seekers do not input a lot information in the on-line form ~\cite{singh2010prospect}, but like to upload their resumes. To the company's side, the recruiters do not like input the job information in the web form either, but prefer to post the whole job description to the web site. Information extraction (IE) is the task of automatically extracting structured information from unstructured and/or semi-structured machine-readable documents.
...
The framework support three styles to create patterns, which gives developers great flexibility. The most common style is define pattern expression in a string, which is much like traditional regular expression. We give an example as follows:
...
In the text of natural language, one concept always have some different expressions. Like bachelor's degree, the simple concept has several expression in job descriptions, like B.S., BA/BS, 4 years degree, and so on. The table ~\ref{tab:multispelling} shows the words that if followed with word "degree", have the semantic value of ``bachelor's degree''. 
